...
test.describe('[Heroku App] [Dynamic Loading]', () => {
    ...
    test('Get by label', async ({ page }) => {
        ...
        // await page.getByTitle("titleValue");
        // await page.getByAltText("textForAlt");
    ...
    /* можно использовать команду npx playwright codegen и на открывшейся странице совершить нужные нам действия. Playwright переведёт наши действия в код. */
    // npx playwright codegen https://anatoly-karpovich.github.io/demo-login-form/
    test('test by npx playwright codegen', async ({ page }) => {
        ...
        await page.locator('#passwordOnRegister').fill('SuperPassword123!');
    ...
    test('Waits with expect', async ({ page }) => {
        ...
        // console.log(text);
        // await expect(page.locator('#finish').getByRole("heading", { level: 4 })).toHaveText("Hello World!", { timeout: 20000 });
    ...
    // для проверки группы экспектов можно использовать expect.soft. Тогда даже если какой-то экспект упадёт, мы перейдём к следующему.
    test('Soft', async ({ page }) => {
        ...
        expect.soft(title).toHaveText("Checkboxes1");

        const form = page.locator("form#checkboxes");
        const formText = await form.innerText();
        const checkboxesTexts = formText!.split("\n").map((el) => el.trim());

        expect.soft(checkboxesTexts[0]).toBe("Checkbox 1");
        expect.soft(checkboxesTexts[1]).toBe("checkbox 2");


        // expect(checkbox1).toHaveText("checkbox 1");
        // expect(checkbox2).toHaveText("checkbox 2");
    });
});